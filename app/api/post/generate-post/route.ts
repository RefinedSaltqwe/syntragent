...
function buildSystemPrompt(channelType?: string, characterLimit?: number) {
  ...
  if (channelType) {
    system_prompt.push(
      `Write for ${channelType}. Match the platform's tone, style, and expected length. and relevant hashtags. `,
    );
  }
  if (characterLimit) {
    system_prompt.push(
      `Must be less than the maximum character limit: ${characterLimit}. `,
    );
  }
