...
import { Wand2, ScanEye, Lightbulb, Image } from "lucide-react";
...
export function EditPostDialog({
  ...
  const [selectedRightTab, setSeletedRightTab] = useState<ActionTabType | null>(
    null,
  );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <div>
          ...
          {/* ── Main panel ── */}
          <div className="w-full flex flex-1 h-full min-h-137.5 max-h-142.5 ">
            {/* Left panel */}
            <div className="flex flex-1 flex-col min-w-0 w-75 pb-5">
              <section
                className="channel--composer relative 
                    flex flex-col px-8 mt-5 min-h-75 
                    h-auto"
              >
                <div className="bg-background rounded-2xl border shadow-sm overflow-hidden min-h-100 flex flex-col p-4">
            ...
            {selectedRightTab && (
              <aside
                className="w-87.5 shrink-0 border-l border-border 
          ...
          <div className="w-full flex items-center justify-between gap-2">
            ...
            >
              {updatePostMutation.isPending &&
                updatePostMutation.variables?.status === POST_STATUS.DRAFT && (
                  <Spinner />
                )}
              Save Draft
            </Button>
